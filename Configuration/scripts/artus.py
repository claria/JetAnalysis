...
def artus():
	...
	if not options.isjson and not options.resume:
		...
		user_conf = custom.config()
		conf = user_conf.get_config()
		exec_name = user_conf.get_exec_name()

		if options.fast:
			conf["InputFiles"] = conf["InputFiles"][fast[0]:fast[1]]
	...
	#	print "Executable {} not found, please compile it first.".format(exec_name)
	#	sys.exit(1)
	if options.batch:  # this works in a specific setup, to be discussed
		...
		if not options.fast and len(conf["InputFiles"])>100:
			...
			try:
				...
				sys.exit(0)
		try:
			subprocess.call([exec_name, options.json])
		except KeyboardInterrupt:
			aborted = True
...
def getoptions(configdir='Configuration/python/', name='artus'):
	...
	if '.py' not in opt.cfg:
		opt.cfg += '.py'
	if '/' not in opt.cfg and not os.path.exists(opt.cfg):
