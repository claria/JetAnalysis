/*
  ...
  This program will add histograms (see note) and Trees from a list of root
  ...
  NOTE1: By default histograms are added. However hadd does not support the case
  where
...
int main(int argc, char** argv) {
  if (argc < 3 || "-h" == std::string(argv[1]) || "--help" == std::string(argv[1])) {
    ...
    std::cout << "If the option -O is used, when merging TTree, the basket "
                 "size is re-optimized" << std::endl;
    std::cout << "If the option -v is used, explicitly set the verbosity "
                 "level; 0 request no output, 99 is the default" << std::endl;
    std::cout << "If the option -n is used, hadd will open at most "
                 "'maxopenedfiles' at once, use 0 to request to use the system "
                 "maximum." << std::endl;
    std::cout << "When -the -f option is specified, one can also specify the "
