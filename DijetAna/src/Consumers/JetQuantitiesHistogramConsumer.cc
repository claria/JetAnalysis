...
void JetQuantitiesHistogramConsumer::Init(setting_type const& settings) {
  ...
  m_h_jet1pt =
      new TH1D("h_jet1pt", "h_jet1pt", settings.GetPtBinning().size() - 1,
  ...
  m_h_jet2pt =
      new TH1D("h_jet2pt", "h_jet2pt", settings.GetPtBinning().size() - 1,
               &settings.GetPtBinning()[0]);
  m_h_jet2pt->Sumw2();
  ...
  // Inclusive Jets
  m_h_incjetpt =
      new TH1D("h_incjetpt", "h_incjetpt", settings.GetPtBinning().size() - 1,
               &settings.GetPtBinning()[0]);
  m_h_incjetpt->Sumw2();

  // Triple differential histogram
  m_h3_jet12rap = new TH3D("h3_jet12rap", "h3_jet12rap",
  ...
  m_h3_ptavg_yio = new TH3D("h3_ptavg_yio", "h3_ptavg_yio",
                            settings.GetRapidityBinning().size() - 1,
                            &settings.GetRapidityBinning()[0],
                            settings.GetRapidityAbsBinning().size() - 1,
                            &settings.GetRapidityAbsBinning()[0],
  ...
  m_h3_ptavg_ysb = new TH3D("h3_ptavg_ysb", "h3_ptavg_ysb",
                            settings.GetRapidityAbsBinning().size() - 1,
                            &settings.GetRapidityAbsBinning()[0],
                            settings.GetRapidityAbsBinning().size() - 1,
                            &settings.GetRapidityAbsBinning()[0],
  ...
  m_h3_ptavg_ysio = new TH3D("h3_ptavg_ysio", "h3_ptavg_ysio",
                             settings.GetRapidityBinning().size() - 1,
                             &settings.GetRapidityBinning()[0],
                             settings.GetRapidityAbsBinning().size() - 1,
                             &settings.GetRapidityAbsBinning()[0],
  ...
  m_h_neutralHadronFraction = new TH1D("h_neutralHadronFraction",
                                       "h_neutralHadronFraction", 50, 0., 2.);
  m_h_chargedHadronFraction = new TH1D("h_chargedHadronFraction",
                                       "h_chargedHadronFraction", 50, 0., 1.);
  m_h_photonFraction =
      new TH1D("h_photonFraction", "h_photonFraction", 50, 0., 2.);
  m_h_electronFraction =
      new TH1D("h_electronFraction", "h_electronFraction", 50, 0., 1.);
  m_h_muonFraction = new TH1D("h_muonFraction", "h_muonFraction", 50, 0., 1.);
  m_h_hfHadronFraction =
      new TH1D("h_hfHadronFraction", "h_hfHadronFraction", 50, 0., 1.);
  m_h_hfEMFraction = new TH1D("h_hfEMFraction", "h_hfEMFraction", 50, 0., 1.);
  m_h_nConstituents =
      new TH1D("h_nConstituents", "h_nConstituents", 250, -0.5, 249.5);
  m_h_nCharged = new TH1D("h_nCharged", "h_nCharged", 250, -0.5, 249.5);
}

void JetQuantitiesHistogramConsumer::ProcessFilteredEvent(
    event_type const& event, product_type const& product,
    setting_type const& settings) {
  double eventWeight =
      product.m_weights.find(settings.GetEventWeight())->second;
  ...
  if (product.m_validJets.size() > 1) {
    ...
    m_h3_jet12rap->Fill(
        boost::math::sign(product.m_jet1Rap) * product.m_jet2Rap,
    ...
    m_h3_ptavg_yio->Fill(
        boost::math::sign(product.m_dijet_yinner) * product.m_dijet_yinner,
        std::abs(product.m_dijet_yinner), product.m_dijet_ptavg, eventWeight);
    m_h3_ptavg_ysb->Fill(std::abs(product.m_dijet_yboost),
                         std::abs(product.m_dijet_ystar), product.m_dijet_ptavg,
                         eventWeight);
    m_h3_ptavg_ysio->Fill(
        product.m_dijet_yinner * boost::math::sign(product.m_dijet_youter),
  ...
  for (auto jet = product.m_validJets.begin(); jet != product.m_validJets.end();
    ...
    m_h_neutralHadronFraction->Fill(
        (*jet)->neutralHadronFraction + (*jet)->hfHadronFraction, eventWeight);
    m_h_chargedHadronFraction->Fill((*jet)->chargedHadronFraction, eventWeight);
    m_h_photonFraction->Fill((*jet)->photonFraction + (*jet)->hfEMFraction,
                             eventWeight);
