...
void GenJetQuantitiesHistogramConsumer::Init(setting_type const& settings) {
  ...
  m_h_genjet1pt = new TH1D("h_genjet1pt", "h_genjet1pt",
  ...
  m_h_genjet2pt = new TH1D("h_genjet2pt", "h_genjet2pt",
                           settings.GetGenPtBinning().size() - 1,
                           &settings.GetGenPtBinning()[0]);
  m_h_genjet2pt->Sumw2();
  ...
  m_h_genjet12rap = new TH2D("h_genjet12rap", "h_genjet12rap",
  ...
  m_h3_genjet12rap = new TH3D("h3_genjet12rap", "h3_genjet12rap",
                              settings.GetRapidityBinning().size() - 1,
                              &settings.GetRapidityBinning()[0],
                              settings.GetRapidityBinning().size() - 1,
                              &settings.GetRapidityBinning()[0],
  ...
  m_h3_genjet12rapsign =
      new TH3D("h3_genjet12rapsign", "h3_genjet12rapsign",
...
void GenJetQuantitiesHistogramConsumer::ProcessEvent(
  ...
  for (auto jet = event.m_genJets->begin(); jet != event.m_genJets->end();
       jet++) {
...
void GenJetQuantitiesHistogramConsumer::ProcessFilteredEvent(
    event_type const& event, product_type const& product,
  ...
  if (event.m_genJets->size() > 0) {
    if (product.m_matchedRecoJets.at(0) != NULL) {
      m_h2GenVsRecoPt->Fill(product.m_matchedRecoJets.at(0)->p4.Pt() /
                                event.m_genJets->at(0).p4.Pt(),
                            event.m_genJets->at(0).p4.Pt(), eventWeight);
      m_h_jet1DeltaR->Fill(
          ROOT::Math::VectorUtil::DeltaR(product.m_matchedRecoJets.at(0)->p4,
  ...
  if (event.m_genJets->size() > 1) {
    if (product.m_matchedRecoJets.at(1) != NULL) {
      m_h_jet2DeltaR->Fill(
          ROOT::Math::VectorUtil::DeltaR(product.m_matchedRecoJets.at(1)->p4,
