...
void JetHltEfficiencyFilter::Init(JetSettings const& settings) {
  ...
  assert(settings.GetTriggerEffThresholds().size() ==
         settings.GetTriggerEffPaths().size() + 1);
  for (size_t i = 0; i < settings.GetTriggerEffPaths().size(); i++) {
    triggerEffThresholds[settings.GetTriggerEffPaths()[i]] =
        std::make_pair(settings.GetTriggerEffThresholds()[i],
                       settings.GetTriggerEffThresholds()[i + 1]);
    std::cout << "Trigger eff. thresholds for path "
              << settings.GetTriggerEffPaths()[i] << " is: ("
              << triggerEffThresholds[settings.GetTriggerEffPaths()[i]].first
              << ", "
              << triggerEffThresholds[settings.GetTriggerEffPaths()[i]].second
              << ")." << std::endl;
  }
}
